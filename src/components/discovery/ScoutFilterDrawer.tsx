import { X, SlidersHorizontal, Check, RotateCcw, MapPin, Gauge, Tag, Ticket, Mail } from 'lucide-react';
...
function Toggle({
  ...
  return (
    <button
      ...
      onClick={onChange}
      className={`relative w-10 h-6 shrink-0 rounded-full transition-colors duration-200 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--theme-brand-accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--theme-bg-base)] ${checked ? 'bg-[var(--theme-brand-accent)]' : 'bg-[var(--theme-border-strong)]'
        }`}
    >
      <span
        className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-[var(--theme-bg-surface)] shadow-sm transition-transform duration-200 ease-out ${checked ? 'translate-x-4' : 'translate-x-0'
          }`}
      />
    </button>
  );
                ...
                {RADIUS_OPTIONS.map((km) => {
                  ...
                  return (
                    <button
                      ...
                      className={`py-1.5 text-[var(--text-2xs)] font-mono rounded-lg transition cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--theme-brand-accent)] ${isSelected
                ...
                {VIBE_THRESHOLDS.map((t) => {
                  ...
                  return (
                    <button
                      ...
                      aria-pressed={isSelected}
                      className={`py-1.5 px-1 rounded-lg transition cursor-pointer text-center leading-tight focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--theme-brand-accent)] ${isSelected
                          ? 'bg-[var(--theme-bg-surface)] text-[var(--theme-text-primary)] shadow-xs'
                          : 'text-[var(--theme-text-muted)] hover:text-[var(--theme-text-primary)]'
                        }`}
                    >
                      <span className={`block text-[var(--text-2xs)] font-sans ${isSelected ? 'font-semibold' : ''}`}>
                        {t.label}
                      </span>
                ...
                {CATEGORIES.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      aria-pressed={isSelected}
                      className={`px-3 py-1.5 rounded-full text-[var(--text-2xs)] transition cursor-pointer flex items-center gap-1.5 border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--theme-brand-accent)] ${isSelected
