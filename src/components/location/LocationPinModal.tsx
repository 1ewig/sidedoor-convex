      ...
      <div className="relative w-full max-w-4xl h-[85vh] max-h-[750px] flex flex-col rounded-3xl bg-[var(--theme-bg-surface)] border border-[var(--theme-border-subtle)] shadow-2xl overflow-hidden">
        ...
        <div className="px-5 py-4 flex items-center justify-between border-b border-[var(--theme-border-subtle)] bg-[var(--theme-bg-surface)] z-10 shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-full bg-[var(--theme-text-primary)]/5 text-[var(--theme-text-primary)] flex items-center justify-center">
        ...
        <div className="relative flex-1 w-full bg-[var(--theme-text-primary)] overflow-hidden min-h-[300px]">
          ...
          {!isMapReady && (
            <div className="absolute inset-0 z-15 flex flex-col items-center justify-center bg-[var(--theme-bg-surface)]/80 backdrop-blur-xs transition-opacity duration-300">
              <Loader2 className="w-7 h-7 text-[var(--theme-text-primary)] animate-spin mb-2" />
          ...
          <div className="absolute top-4 left-4 right-4 sm:right-auto sm:w-80 z-20">
            <div className="relative flex items-center">
              <input
                ...
                className="w-full pl-9 pr-8 py-2.5 rounded-2xl bg-[var(--theme-bg-surface)]/95 backdrop-blur-md border border-[var(--theme-border-subtle)] text-[var(--text-xs)] text-[var(--theme-text-primary)] placeholder-[var(--theme-text-muted)] shadow-lg focus:outline-none focus:border-[var(--theme-text-primary)] font-sans"
              />
              <Search className="w-4 h-4 text-[var(--theme-text-muted)] absolute left-3 pointer-events-none" />
              {isSearching && (
                <Loader2 className="w-3.5 h-3.5 text-[var(--theme-text-primary)] animate-spin absolute right-3 pointer-events-none" />
              )}
            </div>
          ...
          <button
            ...
            className="absolute bottom-4 right-4 z-20 px-3.5 py-2 rounded-2xl bg-[var(--theme-bg-surface)]/95 backdrop-blur-md border border-[var(--theme-border-subtle)] shadow-lg text-[var(--text-xs)] font-medium text-[var(--theme-text-primary)] hover:opacity-80 flex items-center gap-2 transition cursor-pointer"
          >
            {isLocating ? (
              <Loader2 className="w-3.5 h-3.5 text-[var(--theme-text-primary)] animate-spin" />
            ) : (
              <Crosshair className="w-3.5 h-3.5 text-[var(--theme-text-primary)]" />
            )}
        ...
        <div className="px-5 py-4 bg-[var(--theme-bg-surface)] border-t border-[var(--theme-border-subtle)] flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 z-10 shrink-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="flex-1 sm:flex-none px-5 py-2 rounded-xl bg-[var(--theme-text-primary)] hover:bg-[var(--theme-text-secondary)] text-[var(--theme-bg-surface)] text-[var(--text-xs)] font-semibold flex items-center justify-center gap-1.5 shadow-md active:scale-[0.98] transition cursor-pointer"
            >
